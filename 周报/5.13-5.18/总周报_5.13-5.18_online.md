# 自进化 Agent 文献阅读（5.13–5.18，在线版）

时间：5月13日-5月18日

## 本周内容

围绕 **Self-Evolving Agent / Science Agent** 这一方向阅读文献和 repo。

本周主要完成了三件事：

1. 梳理 self-evolving agent 的领域框架，按照 What to Evolve、When to Evolve、How to Evolve、Evaluation 建立阅读索引。
2. 阅读并整理多条技术主线，包括权重级 RL、自反思 prompt evolution、memory/context 闭环、科研端到端系统等。
3. 对 AI Scientist、AutoResearchClaw、SkillNet 等 repo 做初步结构分析。

本周解决的问题是：AI4S 方向如果要做“会持续变好的科研 agent”，这个自进化能力应该落在权重、memory、skill 还是 workflow 上。阅读后的初步观察是，现有工作并不是同一条路线：有的做权重 RL，有的做上下文/记忆更新，有的做 skill library，或者只是把固定科研流程自动化。不过鉴于目前属于入门阅读文献阶段，主要是了解领域不同做法选择。

由此推测是：对科研 agent 来说，短期更可行的入口可能不是直接训练权重，而是先做 **memory / skill / workflow 层面的经验沉淀和复用**。下一步需要继续验证这些经验能否跨任务迁移，以及是否需要引入 RL 来学习“什么时候用这些经验”。
...
首先查阅 [飞书文档 Self-Evolving Agents for Scientific Research: Literature Review](https://complife-lab.feishu.cn/wiki/ViyowMCymiPhR2kOWLocs9Kmndc#outline) 的领域架构。该文档基于行业综述 [A Survey of Self-Evolving Agents](https://arxiv.org/abs/2507.21046)，把 self-evolving agent 拆成四个问题：What to Evolve、When to Evolve、How to Evolve、Evaluation。

这个拆法对后续阅读比较有帮助，因为它避免把所有“会自我改进”的工作混在一起。比如 GRPO / DAPO 主要是在 How to Evolve 里的 RL 分支，GEPA / ACE 更偏 prompt 或 context evolution，ReasoningBank / EvolveR 更偏 memory + policy 的组合，SkillNet 则对应 tool / skill level 的演化。
...
基于综述和飞书文档，把 self-evolving agent 暂时梳理为五条主线。
...
阶段性理解：如果目标是做面向科研任务的 self-evolving agent，单纯沿着 GRPO 变体往下做不一定最合适。科研 agent 的反馈链更长，很多失败不是单步动作失败，而是文献理解、假设构造、实验设计、代码执行、结果解释中的复合错误。因此 memory、skill 和 workflow 层面的演化可能比单纯权重更新更直接。

## 二、基础论文

本周先补了两类前置基础：一类是 agent 的基本交互范式，另一类是 self-evolving agent 的统一综述。这样后面看具体方法时，可以把每篇论文放回“agent 在哪里接收反馈、反馈更新了什么组件、更新发生在任务内还是任务间”这个坐标系里。

### 首先阅读 [ReAct_2022_10](https://arxiv.org/abs/2210.03629) 定义了 LLM Agent 的基本范式 "Thought → Action → Observation" 循环

  ![ReAct Figure 1](assets/react_figure_1.png)

### 阅读 [A Survey of Self-Evolving Agents](https://arxiv.org/abs/2507.21046)
...
这一部分主要关注“模型权重如何通过环境反馈或结果反馈持续更新”。它的优点是能力可以被内化到模型参数中，不依赖外部记忆；问题是训练成本高，而且 reward 设计、credit assignment、长度控制都会影响稳定性。

### 阅读 [GRPO / DeepSeekMath](https://arxiv.org/abs/2402.03300)
...
### 阅读[DAPO](https://arxiv.org/abs/2503.14476)
...
$$0 < \lvert \lbrace o_i \mid \mathrm{isEquivalent}(a, o_i) \rbrace \rvert < G$$
...
### 阅读[WebRL](https://arxiv.org/abs/2411.02337)
...
### 阅读[RAGEN/StarPO](https://arxiv.org/abs/2504.20073)
...
这一条路线的阶段性结论是：权重级 RL 可以带来真实的策略改进，但 agent 场景比数学题更难，因为 reward 往往稀疏、轨迹更长、错误会在多轮交互中累积。StarPO 和 AgentEvolver 的价值在于把 reward 从最终 outcome 往 trajectory / step 级别拆开，这是科研 agent 后续可以借鉴的地方。

## 四、主线 B：不动权重的反思演化路线

第二条主线是不直接更新模型参数，而是更新 prompt、instruction、playbook 或 context。相比 RL，这类方法更轻量，也更容易在现有 agent 系统中落地。它们的核心假设是：LLM 本身已经具备反思和归纳能力，关键是把执行 trace 中的失败原因转成可复用的自然语言规则。

### 阅读[DSPy](https://arxiv.org/abs/2310.03714)

![DSPy](assets/DSPy.png)

### **DSPY** 后继工作 [TextGrad, Nature](https://arxiv.org/abs/2406.07496)
...
### 阅读[GEPA](https://arxiv.org/abs/2507.19457)
#### GEPA 核心技术要点
...
### 阅读[ACE](https://arxiv.org/abs/2510.04618)
...
这一路线给出的启发是：如果要做科研 agent 的长期改进，不一定一开始就训练模型。更可控的起点是维护一套结构化 playbook / memory，把失败实验、有效 prompt、工具使用经验沉淀下来，再通过检索和更新机制影响下一次 run。


## 五、主线 AB 混合：Memory + RL/Context 闭环

这一部分的工作把前两条路线接起来：一方面维护自然语言经验库或记忆库，另一方面通过 RL / SFT / test-time retrieval 让 agent 学会使用这些经验。它们更接近“科研 agent 自进化”的目标，因为科研任务本身就需要跨任务积累经验，而不是每次从零开始。

### 阅读[SEAL](https://arxiv.org/abs/2506.10943)
...
### [ReasoningBank](https://arxiv.org/abs/2509.25140)（不动权重）
### ReasoningBank 技术核心
...
阶段性理解：SEAL、ReasoningBank、EvolveR、AgentEvolver 共同说明了一点，agent 的自进化可以被拆成“产生经验 - 存储经验 - 检索经验 - 使用经验 - 再训练或再更新经验”这个闭环。不同论文差异主要在于经验是否写进权重、是否保留在外部 memory、以及 reward 是否来自最终结果或中间步骤。

## 六、主线 C：科研场景端到端系统

第三类工作直接面向科研场景，目标不是优化单个 benchmark，而是把文献调研、假设生成、实验验证和论文写作连成完整系统。这类工作对本课题最相关，因为它们暴露了科研 agent 的真实难点：长流程、多工具、强验证、跨阶段记忆。

### [InternAgent-1.5 / NovelSeek](https://arxiv.org/abs/2602.08990)
...
这一部分主要看 human-in-the-loop 的工业系统。它和科研 agent 的关系在于：科研任务中很多反馈并不可靠，完全自动闭环容易把错误经验固化，因此需要设计“什么时候问人、问到的知识怎么存、旧知识怎么失效”的机制。
...
除了论文，本周也开始看已有 repo。这里重点不是复现全部功能，而是判断这些系统的核心抽象是什么：是固定 workflow、skill library、memory system，还是具备真正的跨运行自进化能力。
...
在线仓库：[AI-Scientist](https://github.com/SakanaAI/AI-Scientist)
...
在线仓库：[AutoResearchClaw](https://github.com/aiming-lab/AutoResearchClaw)
...
在线仓库：[SkillNet](https://github.com/zjunlp/SkillNet)
...
- 风险：experiments 用 `exec()` 跑 LLM 生成代码（仅研究用，非生产安全）；搜索服务端不开源，依赖 openkg.cn。

## 阶段性总结

本周完成从“论文列表”到“问题地图”的整理。现在可以把 self-evolving agent 暂时理解成三层问题：

1. **更新什么**：权重、prompt、memory、skill、workflow 都可以是进化对象，但它们的成本、稳定性和可解释性不同。
2. **用什么反馈更新**：最终答案 reward、轨迹级 attribution、LLM-as-judge、人工反馈、实验失败日志、文献查新结果，都可以作为反馈信号，但可靠性不同。
3. **如何避免错误经验固化**：科研任务中失败原因复杂，如果没有验证、版本管理和 human-in-the-loop，agent 可能会把错误结论写进 memory 或 skill。

**结合论文和 repo，目前AI4S方向 agent 大多不动权重，而是关注context，memory，skill等，不确定是否可以引入RL。**
