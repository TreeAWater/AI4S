# Downstream deployment task: AI Scientist
...
- Conda prefix: `reference-repos/envs/ai-scientist`
- Result file: `reference-repos/results/01-ai-scientist.md`

Constraints:
- Work only inside `/Users/taw/project/AI4S/reference-repos` for cloned code, environments, logs, scratch files, and notes.
- Use conda for the environment. Create the environment with `conda create --prefix reference-repos/envs/ai-scientist ...`.
...
- Do not require real API keys, GPU-only workloads, long research runs, or paid model calls.
...
5. Run a local smoke test that does not call external LLM APIs. Good candidates include `python -m compileall`, `python launch_scientist.py --help`, import checks, or dependency checks.
