...
class DynamicPlanner:
    def generate(self, messages: list[Message], tool_specs: list[dict], generation_config: LLMGenerationConfig):
        payload = {
            "workflow_id": "wf-human-tools",
            "task_summary": "Review extraction.",
            ...
            "artifact_contracts": {},
            "validation_rules": [],
            "planner_rationale_summary": "Run the solver.",
            "metadata": {"extraction_task": False},
        }
        return LLMRuntimeResponse(action=SubAgentAction(action="final_answer", content=__import__("json").dumps(payload)))
...
def TaskRuntime(**kwargs) -> _TaskRuntime:
    ...
    if isinstance(task_config, TaskConfig) and task_config.dynamic_subagents is None:
        ...
        agents_ref = str(root.parent / "agents.md") if isinstance(root, Path) else str(Path("/tmp/evolab-test-agents") / "human-tools-agents.md")
        kwargs["task_config"] = task_config.model_copy(
            update={
                ...
                "dynamic_subagents": DynamicSubagentsConfig(
                    ...
                    allowed_tool_names=["ask_human", "request_human_review", "notify_human"],
...
def test_plan_aware_node_can_call_human_tool_through_normal_tool_trace(tmp_path: Path):
    ...
    assert result["dynamic_workflows"][0]["runs"][0]["run_ref"] == result["run_ref"]
    assert saved.metadata["dispatch_metadata"]["dynamic_workflow_node"]["node_id"] == "node-solver"
    assert all(
        skill.skill_id not in {"ask_human", "request_human_review", "notify_human"}
        for skill in saved.skill_bundle.skills
    )
