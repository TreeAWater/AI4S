...
def test_output_tools_reject_evolab_internal_paths(tmp_path):
    registry = ToolRegistry()
    register_output_tools(registry, artifact_root=tmp_path)
    runtime = ToolRuntime(registry)
    runtime.prepare(required_tools=["write_report"], allowed_tools=["write_report"], policy=RuntimePolicy())
    ...
    result = runtime.execute(
        ToolCall(call_id="report", name="write_report", arguments={"path": ".evolab/report.md", "content": "bad"})
    )

    assert result.status == "error"
    assert "must not target EvoLab internal .evolab state" in result.content
    assert not (tmp_path / ".evolab" / "report.md").exists()
