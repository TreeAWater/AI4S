...
def test_file_tools_can_exclude_evolab_internal_state(tmp_path):
    (tmp_path / "input.txt").write_text("user", encoding="utf-8")
    state = tmp_path / ".evolab"
    state.mkdir()
    (state / "AGENTS.md").write_text("internal", encoding="utf-8")
    registry = ToolRegistry()
    register_file_tools(registry, base_dir=tmp_path, excluded_roots=[state])
    runtime = ToolRuntime(registry)
    runtime.prepare(required_tools=["list_files", "read_text"], allowed_tools=["list_files", "read_text"], policy=RuntimePolicy())

    listed = runtime.execute(ToolCall(call_id="list", name="list_files", arguments={"root": ".", "recursive": True}))
    blocked = runtime.execute(ToolCall(call_id="read", name="read_text", arguments={"path": ".evolab/AGENTS.md"}))

    assert [item["relative_path"] for item in listed.metadata["files"]] == ["input.txt"]
    assert blocked.status == "error"
    assert "excluded EvoLab internal directory" in blocked.content
