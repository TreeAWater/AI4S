...
def test_completed_observation_with_recoverable_tool_error_counts_as_success(tmp_path):
    graph_path = _graph_path(tmp_path)
    backend = GraphSkillBackend(graph_path, evolution_root=tmp_path / "skill_evolution")
    ...
    result = backend.look_at(
        _observation(
            skill_bundle=_bundle(_skill_ref()),
            final_answer="completed after writing required artifacts",
            tool_trace=_tool_trace_error("recoverable table inspection failed before artifact recovery"),
            metadata={
                "status": "completed",
                "completion_contract": {
    ...
    assert stats["last_status"] == "success"
    proposal_types = {proposal["proposal_type"] for proposal in result.metadata["proposals"]}
    assert "failure_note_update" not in proposal_types
    observation = json.loads((tmp_path / "skill_evolution" / "observations.jsonl").read_text(encoding="utf-8"))
