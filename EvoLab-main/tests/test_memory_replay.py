...
def test_replay_memory_trace_from_clean_native_mem0_lab_validates_backend_scoped_refs(tmp_path: Path):
    ...
    run_clean_demo(Path("dev/configs/demo_v1_ci.yaml"), lab_root)
...
def test_replay_memory_trace_reports_missing_backend_state_record(tmp_path: Path):
    ...
    run_clean_demo(Path("dev/configs/demo_v1_ci.yaml"), lab_root)
    (LabLayout(lab_root).registries_dir / "backend_state" / "states.jsonl").write_text("", encoding="utf-8")
...
def test_replay_memory_trace_accepts_dynamic_task_only_worker_without_agent_memory(tmp_path: Path):
    lab_root = tmp_path / "dynamic-task-only-lab"
    resolver = LabResolver(lab_root)
    trajectory_registry = resolver.trajectory_registry()
    backend_state_registry = resolver.backend_state_registry()
    ...
    backend_state_registry.register_candidate(
        BackendStateRecord(
            ...
            backend_id="task-memory",
            backend_type="memory",
            created_from_task_id="dynamic-task-only",
            created_from_run_ref="subagent-task-only",
            parent_state_refs=["memory://task/v0"],
            metadata={"memory_scope": "task", "memory_scope_id": "task:dynamic-task-only"},
        )
    )
    trajectory_registry.save_subagent_run(
        SubagentRunRecord(
            run_ref="subagent-task-only",
            task_id="dynamic-task-only",
            task_origin=TaskOrigin.HUMAN,
            task_purpose=TaskPurpose.REGRESSION,
            stage_index=0,
            role="dynamic-worker",
            instruction="Exercise dynamic task-only replay.",
            retrieval_request=RetrievalRequest(task_id="dynamic-task-only", role="dynamic-worker", query="prior work"),
            memory_bundle=MemoryBundle(backend_id="task-memory"),
            skill_bundle=SkillBundle(backend_id="skill-local"),
            prompt_messages=[Message(role="user", content="Solve it.")],
            llm_backend_id="llm-api",
            metadata={
                "memory_mode": "task_only",
    ...
    report = replay_memory_trace(lab_root, task_id="dynamic-task-only")

    assert report.ok is True
    assert report.issues == []
    assert [(record.memory_scope, record.backend_id) for record in report.records] == [("task", "task-memory")]
