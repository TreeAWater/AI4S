...
from evolab.backends.evolution import FakeSAGETrainer, PromptOverlayEvolutionTrainer
...
class CountingTrajectoryRegistry(FileTrajectoryRegistry):
    def __init__(self, root: Path | str):
        super().__init__(root)
        self.get_subagent_run_calls = 0
        self.list_subagent_runs_calls = 0

    def get_subagent_run(self, run_ref: str) -> SubagentRunRecord | None:
        self.get_subagent_run_calls += 1
        return super().get_subagent_run(run_ref)

    def list_subagent_runs(self) -> list[SubagentRunRecord]:
        self.list_subagent_runs_calls += 1
        return super().list_subagent_runs()


class RaisingSubagentListRegistry(FileTrajectoryRegistry):
    def list_subagent_runs(self) -> list[SubagentRunRecord]:
        raise AssertionError("subagent registry scan should not be required")
...
def test_task_close_evolution_loads_subagent_records_once_for_many_result_run_refs(tmp_path: Path):
    layout = LabLayout(tmp_path / "lab")
    layout.ensure()
    request = _request()
    trajectory_registry = CountingTrajectoryRegistry(layout.registries_dir / "trajectory")
    backend_state_registry = FileBackendStateRegistry(layout.registries_dir / "backend_state")
    for index in range(3):
        trajectory_registry.save_subagent_run(_subagent_run(run_ref=f"subagent-{index}"))
    ...
    run_refs = scheduler.run(
        request,
        {"run_refs": ["subagent-0", "subagent-1", "subagent-2"]},
    )

    assert len(run_refs) == 3
    assert trajectory_registry.get_subagent_run_calls == 0
    assert trajectory_registry.list_subagent_runs_calls == 1


def test_task_worker_uses_result_run_refs_for_ledger_without_scanning_subagent_registry(tmp_path: Path):
    layout = LabLayout(tmp_path / "lab")
    layout.ensure()
    request = _request()
    _enqueue_task(layout, request)
    trajectory_registry = RaisingSubagentListRegistry(layout.registries_dir / "trajectory")
    runtime = FakeRuntime(
        {
            "task_id": request.task_id,
            "status": "completed",
            "run_refs": ["subagent-result-1", "subagent-result-2"],
        }
    )
    worker = TaskWorker(
        layout=layout,
    ...
    assert result is not None
    ledger = worker.lab_state_registry.get_run_ledger(request.task_id)  # type: ignore[union-attr]
    assert ledger is not None
    assert ledger.subagent_run_refs == ["subagent-result-1", "subagent-result-2"]


def test_task_close_evolution_includes_sanitized_reflector_feedback(tmp_path: Path):
    layout = LabLayout(tmp_path / "lab")
    layout.ensure()
    request = _request()
    trajectory_registry = FileTrajectoryRegistry(layout.registries_dir / "trajectory")
    ...
    run_refs = scheduler.run(
        request,
        {
            ...
            "reflector_evaluation": {
                "score": 0.25,
                "passed": False,
                "metrics": {"precision": 1.0, "recall": 0.2, "f1": 0.333333},
                "summary": "Recall is low because supplementary tables were not fully inspected.",
                "specific_evolution_instructions": [
                    {
                        "stage": "source_discovery",
                        "instruction": "Inspect all structured supplementary sources before validation.",
                    }
                ],
                    ...
                    "false_negative_examples": [
                        {
                            ...
                            "ground_truth_record": {"sequence": "AACCGGTTAACCGGTT"},
                        }
                    ],
                    "false_positive_examples": [
                        {
                            "sequence": "TTTTCCCCAAAAGGGG",
                            "prediction_record": {"sequence": "TTTTCCCCAAAAGGGG", "source_file": "supp.xlsx"},
                        }
                    ],
                    "false_negative_example_count": 1,
                    "false_positive_example_count": 1,
                },
            },
        },
    )

    payload = json.loads(sorted((layout.evolve_queue_dir / "queued").glob("*.json"))[0].read_text(encoding="utf-8"))
    evolution_request = LLMEvolutionRequest.model_validate_json(
        Path(payload["request_payload_uri"]).read_text(encoding="utf-8")
    )

    assert run_refs == [payload["job_id"] if "job_id" in payload else run_refs[0]]
    feedback = evolution_request.metadata["reflector_feedback"]
    assert feedback["metrics"]["recall"] == 0.2
    assert feedback["specific_evolution_instructions"][0]["stage"] == "source_discovery"
    assert feedback["sequence_error_analysis"]["false_negative_examples_redacted"] is True
    assert evolution_request.proposer_input_refs[-1].ref_type == "reflector_feedback"
    serialized = evolution_request.model_dump_json()
    assert "AACCGGTTAACCGGTT" not in serialized
    assert "TTTTCCCCAAAAGGGG" not in serialized


def test_prompt_overlay_evolution_promotes_role_specific_prompt_state(tmp_path: Path):
    layout = LabLayout(tmp_path / "lab")
    layout.ensure()
    request = _request()
    trajectory_registry = FileTrajectoryRegistry(layout.registries_dir / "trajectory")
    backend_state_registry = FileBackendStateRegistry(layout.registries_dir / "backend_state")
    trajectory_registry.save_subagent_run(_subagent_run(role="ExecAgent"))
    scheduler = TaskCloseEvolutionScheduler(
        layout=layout,
        evolve_queue=FileWorkQueue(layout.evolve_queue_dir),
        trajectory_registry=trajectory_registry,
    ...
    EvolveWorker(
        ...
        {"fake-llm": PromptOverlayEvolutionTrainer()},
        backend_state_registry,
        worker_id="evolve-1",
        trajectory_registry=trajectory_registry,
    ).run_once()

    role_state_ref = backend_state_registry.resolve_active_state("fake-llm", role="ExecAgent")
    assert role_state_ref is not None
    assert role_state_ref.startswith("prompt-overlay://fake-llm/execagent/")
    assert backend_state_registry.resolve_active_state("fake-llm") is None
    state = backend_state_registry.get_state(role_state_ref)
    assert state is not None
    overlay = state.metadata["prompt_overlay"]
    assert overlay["role"] == "ExecAgent"
    assert "Inspect all structured supplementary sources" in overlay["system_prompt_append"]
