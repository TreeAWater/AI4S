...
def _runtime(lab_root: Path, *, planner_responses: list[dict], worker_response_count: int) -> TaskRuntime:
    ...
    agents_path = lab_root / "configs" / "agents.md"
    agents_path.parent.mkdir(parents=True, exist_ok=True)
    agents_path.write_text(
        render_agents_markdown(
            {
                "DocumentSurveyAgent": RoleSpec(
                    name="DocumentSurveyAgent",
                    system_prompt="Survey document context.",
                    llm_backend=BackendBinding(backend_id="worker"),
                    allowed_tools=["read_text", "inspect_excel_workbook", "inspect_table", "write_report"],
                )
            }
        ),
        encoding="utf-8",
