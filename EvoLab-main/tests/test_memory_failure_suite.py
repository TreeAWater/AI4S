    ...
    def generate(
        ...
        return LLMRuntimeResponse(
            action=SubAgentAction(
                action="final_answer",
                content=(
                    '{"workflow_id":"wf-memory-failure","task_summary":"Check memory failure handling.",'
                    '"article_context_summary":"unit test","dynamic_subagents":[{"subagent_id":"solver",'
                    '"role_name":"solver","goal":"Check memory failure handling.","system_prompt":"Solve.",'
                    '"input_schema":{"type":"object"},"output_schema":{"type":"object"},'
                    '"allowed_tools":[],"llm_backend_id":"llm-local"}],'
                    '"workflow_nodes":[{"node_id":"node-solver","subagent_id":"solver"}],'
                    '"workflow_edges":[],"artifact_contracts":{},"validation_rules":[],'
                    '"planner_rationale_summary":"Run solver.","metadata":{"extraction_task":false}}'
                ),
            )
        )
...
def TaskRuntime(**kwargs) -> _TaskRuntime:
    ...
    if isinstance(task_config, TaskConfig) and task_config.dynamic_subagents is None:
        root = getattr(kwargs.get("backend_state_registry"), "root", None) or getattr(
            kwargs.get("trajectory_registry"), "root", None
        )
        agents_ref = (
            str(root.parent / "agents.md")
            if isinstance(root, Path)
            else str(Path("/tmp/evolab-test-agents") / "memory-failure-agents.md")
        )
        ...
        kwargs.setdefault("tool_runtime", ToolRuntime(ToolRegistry()))
...
def test_runtime_records_degraded_memory_update_without_registering_state(tmp_path: Path):
    ...
    saved = runtime.trajectory_registry.get_subagent_run(result["run_ref"])
    assert saved is not None
    assert saved.metadata["task_memory_update_result"]["status"] == "degraded"
    assert saved.metadata["task_memory_update_result"]["metadata"]["reason"] == "partial external update"
    assert registry.list_states() == []
...
def test_runtime_preserves_mem0_add_failure_metadata_without_registering_state(tmp_path: Path):
    ...
    assert saved.metadata["task_memory_update_result"]["metadata"]["error_type"] == "extraction_parse"
    assert registry.list_states() == []
