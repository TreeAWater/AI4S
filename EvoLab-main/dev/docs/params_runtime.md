...
- dynamic role-pool planning and task-level worker execution
...
meta_agent:
  system_prompt: |
    Update the role pool when needed before planning.
...
The active role pool comes from `agents_ref`, and dynamic workers use
task-level memory by default. The internal shape is still supported for
deterministic demos, but humans should not need to hand-write
`task_config.roles` for new experiments.
...
The runtime validates planner output against the dynamic role pool and includes
a compact Lab state summary in MetaAgent and planner inputs.
