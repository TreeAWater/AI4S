...
Humans write the task in natural language and point to the active reusable role
pool. The CLI compiles that short config into the internal `TaskRequest` and
`TaskConfig` models used by `TaskRuntime`.
...
each run. Dynamic worker roles use task-level memory only; MetaAgent may keep
stable agent-scope memory.
...
- `meta_agent.memory_backend`: optional MetaAgent memory backend binding. When
  set, use `mem0-meta-memory`; MetaAgent retrieves and updates its own
  agent-scope memory before and after each routing decision.
- `agents_ref`: path to the active role-pool file. For configs under
  `configs/`, use a path relative to that directory, such as
  `agents/scientific_ie_agents.md`.
- `dynamic_subagents`: enabled dynamic workflow planner settings.
...
Do not put top-level `subagents`, task-specific executable roles, fixed DAGs,
`task_config.roles`, or structured task fields into this config. Domain details
belong in natural language task text or in domain package resources.
...
The runtime validates the route against the active role-pool names. It also
...
- `TaskConfig`
- one `RoleSpec` per role loaded from `agents_ref`
- `DynamicSubagentsConfig` with dynamic planning enabled
- `RuntimePolicy` with workflow planning enabled and route-contract metadata
...
The biology extraction config uses the shared scientific IE role pool:

```bash
configs/agents/scientific_ie_agents.md
```

It defines only generic reusable roles:
...
These names are reusable role identities, not biology-specific agents. Dynamic
worker role-pool entries omit role-level agent memory by default; dynamic worker
execution uses task-level memory from `mem0-task-memory`. Biology specificity
appears in the `task` text and in resources under:
...
That legacy verification config reads `OPENAI_API_KEY`, calls
`deepseek/deepseek-v4-flash` through OpenRouter chat completions, and uses
`openai/text-embedding-3-small` through OpenRouter embeddings for native mem0
memory. Migrated dynamic configs configure `mem0-meta-memory` for MetaAgent when
needed and `mem0-task-memory` for dynamic workers.
