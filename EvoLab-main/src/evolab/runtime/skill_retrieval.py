...
def prepare_skill_runtime_context(
    *,
    ...
    skill_bundle = _scope_skill_bundle_for_agent_config(skill_bundle, retrieval_request.metadata)
...
def build_skill_context(skill_bundle: SkillBundle) -> dict[str, Any]:
    ...
    for key in (
        ...
        "agent_config_required_skills",
        "agent_config_filtered_out_skill_ids",
        "agent_config_skill_scope_warning",
    ):
...
def _scope_skill_bundle_for_agent_config(
    skill_bundle: SkillBundle,
    request_metadata: dict[str, Any],
) -> SkillBundle:
    required = _string_list(request_metadata.get("agent_config_required_skills"))
    if not required:
        return skill_bundle
    ...
    kept = [
        skill
        for skill in skill_bundle.skills
        if skill.skill_id.casefold() in required_set or skill.name.casefold() in required_set
    ]
    metadata = {**skill_bundle.metadata, "agent_config_required_skills": required}
    if not kept:
        return skill_bundle.model_copy(
            update={
                "metadata": {
                    **metadata,
                    "agent_config_skill_scope_warning": "required_skills matched no retrieved skills; original bundle retained",
                }
            }
        )
    kept_ids = {skill.skill_id for skill in kept}
    filtered = [skill.skill_id for skill in skill_bundle.skills if skill.skill_id not in kept_ids]
    trace = metadata.get("retrieval_trace")
    if isinstance(trace, dict):
        metadata["retrieval_trace"] = _filtered_retrieval_trace(trace, kept_ids)
    return skill_bundle.model_copy(
        update={
            ...
            "metadata": {
                **metadata,
                "agent_config_filtered_out_skill_ids": filtered,
            },
        }
    )
