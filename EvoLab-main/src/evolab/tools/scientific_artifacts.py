...
def build_candidate_records(arguments: dict[str, Any], *, artifact_root: Path | None = None) -> ToolResult:
    ...
    for index, candidate_row in enumerate(rows_payload.get("candidate_rows", []), start=1):
        ...
        if not isinstance(sequence_fields, list) or not sequence_fields:
            continue
        sequence_field = _best_sequence_field(
            sequence_fields,
            extraction_profile=extraction_profile,
        ...
        record = {
            ...
            "source_classification": _source_classification_from_candidate_row(
                candidate_row,
                sequence_field=str(sequence_field.get("field") or ""),
                extraction_profile=extraction_profile,
            ),
...
def _best_sequence_field(
    sequence_fields: list[Any],
    *,
    ...
    scored = sorted(
        (
            (
                field,
                _promoter_sequence_field_score(
...
def _promoter_sequence_field_score(field_name: str, *, candidate_row: dict[str, Any] | None = None) -> int:
    ...
    if normalized in {"promoter_sequence", "promoter_sequences", "pro_seq"}:
        score += 8
    if normalized in {"promoter", "promoters", "regulatory", "regulatory_element", "regulatory_sequence"}:
        if candidate_row is None or _bare_target_sequence_field_allowed(candidate_row):
            score += 6
    if "sequence" in normalized and "generated" not in normalized and "predicted" not in normalized:
        if candidate_row is None or _generic_sequence_field_allowed(candidate_row):
            score += 2
    if (normalized.endswith("_sequence") or normalized.endswith("_seq")) and (
        candidate_row is None or _generic_sequence_field_allowed(candidate_row)
    ):
        score += 2
    if candidate_row is not None and _field_name_is_sequence_value(normalized) and _generic_sequence_field_allowed(candidate_row):
        score += 4
...
def _field_name_is_sequence_value(field_name: str) -> bool:
    sequence = _normalize_sequence(field_name)
    return len(sequence) >= 30 and _is_dna_like(sequence, min_length=30)


def _generic_sequence_field_allowed(candidate_row: dict[str, Any]) -> bool:
    """Require target context before treating generic *_seq fields as primary component fields."""

    context_text = _candidate_row_table_context_text(candidate_row)
    row = candidate_row.get("row")
    row_text = json.dumps(row, sort_keys=True, default=str).casefold() if isinstance(row, dict) else ""
    target_terms = ("promoter", "regulatory", "sigma", "u6", "polii", "pol_ii")
    return any(term in context_text or term in row_text for term in target_terms)


def _bare_target_sequence_field_allowed(candidate_row: dict[str, Any]) -> bool:
    """Allow high-confidence bare target fields without opening broad reference tables."""

    if _candidate_row_references_external_dataset(candidate_row):
        return False
    if _candidate_row_matches_article_provenance_reference(candidate_row):
        return True
    context_text = _candidate_row_table_context_text(candidate_row)
    return any(term in context_text for term in ("promoter", "regulatory", "sigma", "u6", "polii", "pol_ii"))


def _candidate_row_references_external_dataset(candidate_row: dict[str, Any]) -> bool:
    source_file = candidate_row.get("source_file")
    if not isinstance(source_file, str) or not source_file:
        return False
    provenance_tokens = _article_provenance_tokens_for_source(source_file)
    if not provenance_tokens:
        return False
    citation_tokens = _external_citation_tokens(_candidate_row_table_context_text(candidate_row))
    return any(token and token not in provenance_tokens for token in citation_tokens)


def _candidate_row_matches_article_provenance_reference(candidate_row: dict[str, Any]) -> bool:
    source_file = candidate_row.get("source_file")
    if not isinstance(source_file, str) or not source_file:
        return False
    provenance_tokens = _article_provenance_tokens_for_source(source_file)
    if not provenance_tokens:
        return False
    citation_tokens = _external_citation_tokens(_candidate_row_table_context_text(candidate_row))
    return any(token and token in provenance_tokens for token in citation_tokens)


def _candidate_row_table_context_text(candidate_row: dict[str, Any]) -> str:
    values = [
        candidate_row.get("sheet_name"),
        candidate_row.get("table_id"),
        candidate_row.get("table_context"),
    ]
    return " ".join(json.dumps(value, sort_keys=True, default=str) for value in values if value).casefold()


def _external_citation_tokens(text: str) -> set[str]:
    tokens = set()
    for match in re.finditer(r"\b([A-Za-z][A-Za-z-]*(?:\s+[A-Za-z][A-Za-z-]*)?)\s+et\s+al\b", text):
...
def _article_provenance_tokens_for_source(source_file: str) -> frozenset[str]:
    source_path = Path(source_file).expanduser()
    roots = [source_path.parent]
    roots.extend(source_path.parents)
    article_root = next((root for root in roots if (root / "manifest.json").exists()), source_path.parent)
    texts = [article_root.name, source_path.name]
    manifest_path = article_root / "manifest.json"
    if manifest_path.exists():
        ...
        if isinstance(manifest, dict):
            for key in ("article_name", "main_pdf", "article_input_dir", "article_output_dir"):
            ...
            if isinstance(conversions, list):
                for conversion in conversions:
                    if not isinstance(conversion, dict):
                        continue
                    for key in ("input_path", "rel_path", "output_md"):
    ...
    for text in texts:
        ...
        for index in range(len(pieces) - 1):
            tokens.add(pieces[index] + pieces[index + 1])
...
def validate_candidate_records(arguments: dict[str, Any], *, artifact_root: Path | None = None) -> ToolResult:
    ...
    for record in records:
        if not isinstance(record, dict):
            continue
        source_classification = _source_classification_from_record(record, extraction_profile=extraction_profile)
        issues = _record_validation_issues(record, extraction_profile=extraction_profile)
        if source_classification.get("decision") == "reject":
            issues.append(
                "source classified as "
                f"{source_classification.get('category')}: "
                + "; ".join(str(reason) for reason in source_classification.get("reasons", [])[:2])
            )
        validated = dict(record)
        validated["source_classification"] = source_classification
        validated["validation_issues"] = issues
        if issues:
            validated["status"] = "rejected"
            rejected.append(validated)
        elif source_classification.get("decision") == "review":
            validated["status"] = "human_review"
            validated["review_reasons"] = source_classification.get("reasons", [])
    ...
    payload = {
        ...
        "validation_policy": [
            ...
            "source/table context is classified before final acceptance",
            "ambiguous but potentially relevant sources are routed to human_review instead of final records",
    ...
    return _write_artifact_result(
        ...
        content=f"validated {len(records)} records ({len(accepted)} accepted, {len(review)} review)",
    )
...
def serialize_final_records(arguments: dict[str, Any], *, artifact_root: Path | None = None) -> ToolResult:
    ...
    if isinstance(records_path_value, str) and records_path_value:
        records = _accepted_records_from_record_artifact(Path(records_path_value).expanduser())
    else:
...
def _select_tables_for_extraction(
    ...
    *,
    ...
    for source_tables in by_source.values():
        ...
        selected_ids = {id(candidate) for candidate in candidates}
    ...
    return selected, {
        ...
        "policy": (
            "precision-oriented primary component table selection; keeps all extractable primary "
            "component tables per source and stages secondary assay/design/detail tables"
        ),
    }
...
def _table_selection_signals(table: dict[str, Any], *, extraction_profile: str | None) -> dict[str, Any]:
    headers = _headers_for_table_selection(table)
    header_text = " ".join(headers).casefold()
    context = _table_context(table)
    context_text = _semantic_context_text(context)
    profile = (extraction_profile or "").casefold()
    reasons: list[str] = []
    score = 0.0
    hard_reject = False
    source_classification = _source_classification_from_table(
        table,
        headers=headers,
        extraction_profile=extraction_profile,
    )
    ...
    source_decision = source_classification.get("decision")
    source_category = source_classification.get("category")
    if source_decision == "accept":
        score += 3
        reasons.append(f"source classified as {source_category}")
    elif source_decision == "review":
        score -= 2
        reasons.append(f"source requires review: {source_category}")
    elif source_decision == "reject":
        hard_reject = True
        reasons.append(f"source rejected: {source_category}")
...
def _table_is_prediction_only(table: dict[str, Any]) -> bool:
    ...
    empirical_text = re.sub(r"predict(?:ed)?[_\s-]*strength\w*", "", text)
    prediction_terms = (
        "generated_sequences",
        "generated sequence",
        "predicted_value",
    ...
    return any(term in text for term in prediction_terms) and not any(term in empirical_text for term in measurement_terms)
...
def _headers_for_table_selection(table: dict[str, Any]) -> list[str]:
    headers = _string_list(table.get("headers"))
    rows = [_list_row(row) for row in (table.get("sample_rows") or table.get("rows") or [])]
    if headers and not _headers_look_data_like(headers):
        return headers
    if rows:
        detected = _detect_header_lightweight(rows)
        detected_headers = _string_list(detected.get("headers"))
        if detected_headers:
...
def _table_text(table: dict[str, Any]) -> str:
    ...
    values = [
        ...
        _local_table_id(table.get("table_id")),
        table.get("reason"),
        table.get("surrounding_context"),
        table.get("section_heading"),
        table.get("section_label"),
        _semantic_context_text(table_context),
    ]
    return " ".join(json.dumps(value, sort_keys=True, default=str) for value in values if value).casefold()


def _source_classification_from_table(
    table: dict[str, Any],
    *,
    headers: list[str] | None = None,
    extraction_profile: str | None,
) -> dict[str, Any]:
    return _classify_source_context(
        headers=headers or _headers_for_table_selection(table),
        row={},
        context=_table_context(table),
        sequence_field="",
        extraction_profile=extraction_profile,
...
def _source_classification_from_candidate_row(
    candidate_row: dict[str, Any],
    *,
    sequence_field: str,
    extraction_profile: str | None,
    ...
    if candidate_row.get("sheet_name") not in (None, ""):
        context["sheet_name"] = candidate_row.get("sheet_name")
    if candidate_row.get("table_id") not in (None, ""):
        context["table_id"] = candidate_row.get("table_id")
        context["local_table_id"] = _local_table_id(candidate_row.get("table_id"))
    row = candidate_row.get("row") if isinstance(candidate_row.get("row"), dict) else {}
    return _classify_source_context(
        headers=[str(key) for key in row],
        row=row,
        context=context,
...
def _source_classification_from_record(record: dict[str, Any], *, extraction_profile: str | None) -> dict[str, Any]:
    context: dict[str, Any] = {}
    table_context = record.get("source_table_context")
    if isinstance(table_context, dict):
        context.update(table_context)
    if record.get("source_sheet") not in (None, ""):
        context["sheet_name"] = record.get("source_sheet")
    if record.get("source_table") not in (None, ""):
        context["table_id"] = record.get("source_table")
        context["local_table_id"] = _local_table_id(record.get("source_table"))
    evidence_row = _record_evidence_row(record)
    return _classify_source_context(
        headers=[str(key) for key in evidence_row],
        row=evidence_row,
        context=context,
        sequence_field=str(record.get("sequence_field") or ""),
        extraction_profile=extraction_profile,
    )


def _classify_source_context(
    *,
    headers: list[str],
    row: dict[str, Any],
    context: dict[str, Any],
    ...
    if profile not in {"promoter", "promoter_sequence", "regulatory_sequence"}:
        return {
            "category": "generic_sequence_source",
            "decision": "accept",
            "reasons": ["no profile-specific source classifier configured"],
            "signals": {},
        }

    header_text = " ".join(str(header).casefold() for header in headers)
    row_text = json.dumps(row, sort_keys=True, default=str).casefold() if row else ""
    context_text = _semantic_context_text(context)
    sequence_field_text = str(sequence_field or "").casefold()
    all_text = " ".join([header_text, row_text, context_text, sequence_field_text])
    ...
    generated_terms = ("generated", "design-only", "designed candidate", "candidate design")
    background_terms = ("regulondb", "database", "reference dataset", "background", "genome database", "jaspar")
    metadata_terms = ("metadata", "readme", "read me", "taxonomy", "sample accession")
    primer_terms = ("primer", "oligo", "adapter", "barcode", "linker", "restriction", "spacer")
    construct_terms = ("construct", "scaffold", "plasmid", "vector", "backbone", "cassette")
    identity_terms = ("name", "id", "label", "part", "variant", "component", "library")

    has_target = any(term in all_text for term in target_terms)
    has_sequence = bool(_target_sequence_headers(headers, extraction_profile=profile)) or any(
        term in header_text or term in sequence_field_text for term in ("sequence", "seq", "dna")
    )
    has_identity = any(term in header_text for term in identity_terms)
    has_measurement = any(term in all_text for term in measurement_terms)
    has_prediction = any(term in all_text for term in prediction_terms)
    has_generated = any(term in all_text for term in generated_terms)
    has_background = any(term in context_text for term in background_terms) or bool(
        re.search(r"\b[a-z]*db\b", context_text)
    )
    has_metadata = any(term in context_text for term in metadata_terms)
    sequence_like_headers = " ".join(
        header for header in headers if any(token in header.casefold() for token in ("sequence", "seq", "dna"))
    ).casefold()
    artifact_source_text = " ".join([sequence_like_headers, sequence_field_text, context_text])
    has_primer = any(term in artifact_source_text for term in primer_terms)
    has_construct = any(term in artifact_source_text for term in construct_terms)

    signals = {
        "has_target_semantics": has_target,
        "has_sequence_column": has_sequence,
        "has_identity_column": has_identity,
        "has_measurement_evidence": has_measurement,
        "has_prediction_only_signals": has_prediction,
        "has_generated_or_design_signals": has_generated,
        "has_background_database_signals": has_background,
        "has_broad_metadata_signals": has_metadata,
        "has_primer_or_oligo_signals": has_primer,
        "has_construct_or_scaffold_signals": has_construct,
    }

    if has_primer and not (has_target and has_measurement):
        return _source_classification(
            "primer_or_oligo_source",
            "reject",
            ["source context describes primer/oligo/barcode/adapter artifacts without target measurement evidence"],
            signals,
        )
    if has_construct and not (has_target and has_measurement):
        return _source_classification(
            "construct_or_scaffold_source",
            "reject",
            ["source context describes construct/scaffold/vector/plasmid artifacts without target measurement evidence"],
            signals,
        )
    if has_background and not (has_target and has_measurement):
        return _source_classification(
            "background_database_source",
            "reject",
            ["source context appears to be a background/reference database rather than current component evidence"],
            signals,
        )
    if has_prediction and not has_measurement:
        return _source_classification(
            "prediction_only_source",
            "reject",
            ["source has prediction/model score signals without measurement or characterization evidence"],
            signals,
        )
    if has_generated and not has_measurement:
        return _source_classification(
            "design_or_generated_library_source",
            "reject",
            ["source has generated/design-only signals without measurement or characterization evidence"],
            signals,
        )
    if has_metadata and not has_measurement:
        return _source_classification(
            "broad_metadata_source",
            "review",
            ["source appears to be broad metadata without direct measurement evidence"],
            signals,
        )
    if has_target and has_sequence and has_measurement:
        return _source_classification(
            "experimentally_validated_promoter_source",
            "accept",
            ["source has target semantics, sequence evidence, and measurement/characterization evidence"],
            signals,
        )
    if has_target and has_sequence and has_identity and not (has_prediction or has_generated):
        return _source_classification(
            "ambiguous_needs_review",
            "review",
            ["source has target sequence and identity columns but lacks explicit measurement evidence"],
            signals,
        )
    if has_sequence and has_measurement:
        return _source_classification(
            "ambiguous_needs_review",
            "review",
            ["source has sequence and measurement evidence but lacks target component semantics"],
            signals,
        )
    return _source_classification(
        "ambiguous_needs_review",
        "review",
        ["source lacks enough target-specific experimental evidence for final acceptance"],
        signals,
    )


def _source_classification(category: str, decision: str, reasons: list[str], signals: dict[str, bool]) -> dict[str, Any]:
    return {
        "category": category,
        "decision": decision,
        "reasons": reasons,
...
def _table_context(table: dict[str, Any]) -> dict[str, Any]:
    context = {
        "table_id": table.get("table_id"),
        "local_table_id": _local_table_id(table.get("table_id")),
        "sheet_name": table.get("sheet_name"),
        "table_index": table.get("table_index"),
...
def _local_table_id(value: Any) -> str:
    text = str(value or "")
    if "::" in text:
...
def _semantic_context_text(context: Any) -> str:
    """Return table/source context text without source filenames or filesystem paths."""

    if not isinstance(context, dict):
        return json.dumps(context, sort_keys=True, default=str).casefold() if context else ""
    values = []
    for key in (
        "local_table_id",
        "sheet_name",
        ...
        if value not in (None, "", []):
            values.append(value)
    if not values and context.get("table_id"):
        values.append(_local_table_id(context.get("table_id")))
    return " ".join(json.dumps(value, sort_keys=True, default=str) for value in values if value).casefold()
...
def _header_row_score(row: list[Any]) -> float:
    non_empty = [cell for cell in row if cell is not None and str(cell).strip()]
    if not non_empty:
        return -10.0
    normalized = [_normalize_header(cell) for cell in non_empty]
    text = " ".join(normalized).casefold()
    semantic_terms = (
        "sequence",
        "seq",
        "promoter",
    ...
    semantic_hits = sum(1 for term in semantic_terms if term in text)
    numeric_like = sum(1 for cell in non_empty if _looks_numeric_like(cell))
    dna_like = sum(1 for cell in non_empty if _cell_looks_like_long_sequence(cell))
    long_text = sum(1 for cell in non_empty if len(str(cell)) > 80)
    string_count = sum(isinstance(cell, str) for cell in non_empty)
    score = 0.0
    score += semantic_hits * 3.0
    score += min(len(non_empty), 8) * 0.2
    score += string_count * 0.15
    score -= numeric_like * 1.2
    score -= dna_like * 4.0
    score -= long_text * 0.6
    if len(non_empty) <= 1:
        score -= 2.0
    if semantic_hits == 0 and (dna_like or numeric_like >= max(2, len(non_empty) // 2)):
        score -= 6.0
    return score
...
def _looks_numeric_like(value: Any) -> bool:
    ...
    if not text:
        return False
    return bool(re.fullmatch(r"[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:e[-+]?\d+)?%?", text, flags=re.I))


def _cell_looks_like_long_sequence(value: Any) -> bool:
...
def _rows_for_candidate_table(table: dict[str, Any], *, max_rows: int) -> list[list[Any]]:
    ...
    if file_type == "csv_tsv":
        ...
        return [_list_row(row) for row in result.metadata.get("rows", [])]
    if file_type == "markdown":
        plain_text_block = table.get("plain_text_table_block")
        if isinstance(plain_text_block, dict):
            args: dict[str, Any] = {
                "path": str(source_file),
                "start_row": 0,
                "end_row": max_rows + 1,
            }
            caption = plain_text_block.get("caption")
            if isinstance(caption, str) and caption.strip():
                args["table_caption"] = caption
            result = read_table_slice(args)
            rows = [_list_row(row) for row in result.metadata.get("rows", [])]
            if rows:
                return rows
...
def _data_rows_after_header(table: dict[str, Any], rows: list[list[Any]]) -> list[list[Any]]:
    headers = _string_list(table.get("headers"))
    if headers and _headers_look_data_like(headers):
        detected = _detect_header_lightweight(rows)
        detected_index = detected.get("header_row_index")
        if isinstance(detected_index, int):
            return rows[detected_index + 1 :]
...
def _headers_look_data_like(headers: list[str]) -> bool:
    if not headers:
        return False
    text = " ".join(headers).casefold()
    semantic_terms = (
        "sequence",
        "seq",
        "promoter",
    ...
    semantic_hits = sum(1 for term in semantic_terms if term in text)
    dna_like = sum(1 for header in headers if _cell_looks_like_long_sequence(header))
    numeric_like = sum(1 for header in headers if _looks_numeric_like(header.replace("_", ".")))
    long_headers = sum(1 for header in headers if len(header) > 60)
    if dna_like:
        return True
    if semantic_hits == 0 and (numeric_like >= max(2, len(headers) // 2) or long_headers >= max(2, len(headers) // 3)):
        return True
    return False
...
def _profile_validation_issues(record: dict[str, Any], *, extraction_profile: str | None) -> list[str]:
    ...
    target_text = _record_target_semantic_text(record)
    sequence_field = str(record.get("sequence_field") or "").casefold()
    target_terms = ("promoter", "regulatory", "sigma", "u6", "polii", "pol_ii")
    if not any(term in target_text for term in target_terms):
        return ["missing target-component semantics for promoter profile"]
    if _sequence_field_is_ambiguous_target_alias(sequence_field) and not any(
        term in target_text for term in target_terms
    ):
        return [f"sequence field is ambiguous without target-component evidence: {sequence_field}"]
    ...
    if _is_external_reference_dataset_record(record):
        return ["external/reference dataset row is not current-study primary component evidence"]
    if _is_model_evaluation_detail_record(record):
        return ["model-evaluation residual/error row is not a primary component record"]
    if _is_raw_replicate_library_detail_record(record):
        return ["raw replicate library row lacks stable component identity for final primary records"]
    return []


def _record_target_semantic_text(record: dict[str, Any]) -> str:
    """Text used to prove target-component semantics without relying on source path names."""

    evidence_source = record.get("evidence_source")
    evidence_source_without_path = {}
    if isinstance(evidence_source, dict):
        evidence_source_without_path = {
            key: value
            for key, value in evidence_source.items()
            if key not in {"path", "source_file"} and value not in (None, "")
        }
        if evidence_source_without_path.get("table_id"):
            evidence_source_without_path["table_id"] = _local_table_id(evidence_source_without_path.get("table_id"))
        table_context = evidence_source_without_path.get("table_context")
        if isinstance(table_context, dict):
            evidence_source_without_path["table_context"] = _semantic_context_text(table_context)
    source_context = record.get("source_table_context")
    if isinstance(source_context, dict):
        source_context = _semantic_context_text(source_context)
    values = [
...
def _sequence_field_is_ambiguous_target_alias(sequence_field: str) -> bool:
    if not sequence_field:
        return False
    strong_terms = ("promoter", "regulatory", "sigma", "u6", "polii", "pol_ii")
    if any(term in sequence_field for term in strong_terms):
        return False
    return sequence_field in {"sequence", "seq", "dna", "p_seq", "sequence_value"} or sequence_field.endswith(
        ("_seq", "_sequence")
    )
...
def _is_external_reference_dataset_record(record: dict[str, Any]) -> bool:
    ...
    if not provenance_tokens:
        return False
    citation_tokens = _external_citation_tokens(_record_table_context_text(record))
    return any(token and token not in provenance_tokens for token in citation_tokens)


def _is_model_evaluation_detail_record(record: dict[str, Any]) -> bool:
    text = _record_semantic_text(record)
    error_terms = ("abs_residual", "residual_2", "mae", "mse", "slope")
    model_terms = ("predicted", "prediction", "model")
    observed_terms = ("observed", "measured")
    has_model_and_observed = any(term in text for term in model_terms) and any(term in text for term in observed_terms)
    if not has_model_and_observed:
        return False
    if any(term in text for term in error_terms):
        return True
    return not _record_matches_article_provenance_reference(record)


def _record_matches_article_provenance_reference(record: dict[str, Any]) -> bool:
    source_file = record.get("source_file")
    if not isinstance(source_file, str) or not source_file:
        return False
    provenance_tokens = _article_provenance_tokens_for_source(source_file)
...
def _is_raw_replicate_library_detail_record(record: dict[str, Any]) -> bool:
    text = _record_semantic_text(record)
    if "biological_replicate" not in text and "technical_replicate" not in text:
        return False
    row = _record_evidence_row(record)
    if not row:
        return False
    label_terms = ("name", "id", "label", "part", "variant", "component", "construct")
    has_identity_column = any(any(term in str(field).casefold() for term in label_terms) for field in row)
    if has_identity_column:
        return False
    return True


def _record_evidence_row(record: dict[str, Any]) -> dict[str, Any]:
...
def _record_table_context_text(record: dict[str, Any]) -> str:
    source_context = record.get("source_table_context")
    if isinstance(source_context, dict):
        source_context = _semantic_context_text(source_context)
    evidence_source = record.get("evidence_source")
    if isinstance(evidence_source, dict):
        evidence_source = {
            key: (_local_table_id(value) if key == "table_id" else value)
            for key, value in evidence_source.items()
            if key not in {"path", "source_file"}
        }
        if isinstance(evidence_source.get("table_context"), dict):
            evidence_source["table_context"] = _semantic_context_text(evidence_source["table_context"])
    values = [
        record.get("source_sheet"),
        _local_table_id(record.get("source_table")),
        source_context,
        evidence_source,
    ]
    return " ".join(json.dumps(value, sort_keys=True, default=str) for value in values if value).casefold()
...
def _accepted_records_from_record_artifact(path: Path) -> list[dict[str, Any]]:
    if path.suffix.casefold() == ".jsonl":
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            if isinstance(item, dict) and item.get("status") not in {"rejected", "human_review", "low_confidence"}:
                records.append(item)
        return records
    return _accepted_records_from_payload(_load_json(path))
...
def _write_artifact_result(
    ...
    *,
    ...
    warnings: list[str] = []
    existing_payload = _load_existing_json_payload(path)
    if _should_preserve_existing_handoff_artifact(
        existing_payload,
        payload,
        artifact_kind=artifact_kind,
        filename=filename,
    ):
        payload = existing_payload if isinstance(existing_payload, dict) else payload
        warnings.append(
            "preserved existing higher-coverage handoff artifact; smaller candidate output was not allowed to overwrite it"
        )
    else:
    ...
    return ToolResult(
        call_id=call_id,
        status="ok",
        content=content if not warnings else f"{content}; {'; '.join(warnings)}",
        artifact_refs=[
            ArtifactRef(
                uri=str(path),
                type="dataset",
...
def _load_existing_json_payload(path: Path) -> dict[str, Any] | None:
    if not path.exists():
        return None
    try:
...
def _should_preserve_existing_handoff_artifact(
    ...
    *,
    ...
    if existing_payload is None:
        return False
    if artifact_kind == "validated_records" or filename == "validated_records.json":
        # Validation artifacts are policy decisions, not coverage artifacts. A newer stricter
        # validation may intentionally accept fewer records and must be allowed to replace an
        # older broad validation result.
        return False
    comparable = {
        "candidate_rows": "row_count",
        "candidate_records": "record_count",
    }
    key = comparable.get(artifact_kind)
    if key is None and filename in {"candidate_rows.json", "candidate_records.json", "validated_records.json"}:
        key = {
            "candidate_rows.json": "row_count",
            "candidate_records.json": "record_count",
            "validated_records.json": "accepted_count",
        }[filename]
    if key is None:
        return False
    existing_count = existing_payload.get(key)
    new_count = new_payload.get(key)
