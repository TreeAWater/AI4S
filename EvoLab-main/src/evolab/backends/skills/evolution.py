...
def _run_status(observation: dict[str, Any], tool_calls: list[dict[str, Any]]) -> SkillRunStatus:
    ...
    if explicit:
        ...
        if normalized == "partial":
            return "partial"
        if normalized in {"budget_exceeded", "interrupted"}:
            return "partial"
    completion_contract = _dict(metadata.get("completion_contract"))
    if completion_contract.get("ready_for_task_end") is True and completion_contract.get("produced_required_outputs") is True:
        return "success"
