...
from evolab.runtime.task_runtime import TaskRuntime, _subagent_completion_contract
...
def test_default_task_runtime_records_interrupted_subagent_for_early_runtime_failure(tmp_path: Path):
    ...
    worker = TaskWorker(layout=layout, worker_id="worker-1", task_config=task_config)
    worker.startup()
...
def test_default_task_runtime_runs_memory_skill_llm_and_records_updates(tmp_path: Path):
    ...
    assert len(memory.search_requests) == 2
    agent_request, task_request = memory.search_requests
    assert agent_request.query == request.goal
    assert agent_request.task_origin == TaskOrigin.HUMAN
    assert agent_request.role == "solver"
    assert agent_request.filters["memory_scope"] == "agent"
    assert agent_request.filters["memory_scope_id"] == "agent:solver"
    ...
    assert len(skill.get_requests) == 1
    assert skill.get_requests[0] == agent_request
    assert len(llm.calls) == 1
    prompt_text = "\n".join(message.content for message in llm.calls[0][0])
    assert "Agent Memory" in prompt_text
    assert "Task Memory" in prompt_text
    assert "Previous result: catalyst A improved yield." in prompt_text
    ...
    assert (add_task_id, add_role) == (request.task_id, "solver")
    assert "Use catalyst A." in add_messages[-1].content
    add_task_id, add_role, add_messages = memory.add_calls[1]
    ...
    assert saved.memory_bundle.metadata["agent_memory_state_ref"] == "memory-state-before"
    assert saved.memory_bundle.metadata["task_memory_state_ref"] == "memory-state-before"
    ...
    assert saved.metadata["agent_memory_update_result"] == {
        "status": "updated",
        "state_ref": "memory-state-after",
    }
...
def test_default_task_runtime_executes_single_tool_call_and_records_trace(tmp_path: Path):
    ...
    assert llm_calls[0].run_ref == result["run_ref"]
    assert llm_calls[0].backend_id == "llm-local"
    assert llm_calls[0].output_messages == [
        Message(
            role="assistant",
            content="",
            metadata={
    ...
    assert llm_calls[1].input_messages[-1].role == "tool"
    assert llm_calls[1].input_messages[-1].content == "found catalyst"
    assert llm_calls[1].output_messages[0].content == "Use the lookup result."
...
def test_default_task_runtime_saves_partial_subagent_before_raising_guard_failure(tmp_path: Path):
    ...
    with pytest.raises(RuntimeError, match="JSONL record_count is below the required minimum"):
        runtime.run(request)
...
def test_default_task_runtime_subagent_report_includes_internal_dag_skill_and_coverage_summaries(tmp_path: Path):
    ...
    assert report.metadata["internal_dag"]["node_count"] == 1
    assert report.metadata["internal_dag"]["nodes"][0]["skill_id"] == "skill-1"
    assert report.metadata["retrieved_skills"][0]["skill_id"] == "skill-1"
    assert report.metadata["prepared_tools_by_node"][0]["tool_names"] == ["lookup"]
...
def test_default_task_runtime_reads_and_writes_agent_and_task_memory_scopes(tmp_path: Path):
    ...
    assert result["task_id"] == request.task_id
    assert len(agent_memory.search_requests) == 1
    agent_request = agent_memory.search_requests[0]
    assert agent_request.task_id == request.task_id
    assert agent_request.role == "solver"
    assert agent_request.query == request.goal
    assert agent_request.filters["memory_scope"] == "agent"
    assert agent_request.filters["memory_scope_id"] == "agent:solver"
    ...
    assert "Agent Memory" in prompt_text
    assert "Task Memory" in prompt_text
    assert "solver prefers catalyst A" in prompt_text
    assert "surveyor ruled out catalyst B" in prompt_text

    assert len(agent_memory.add_calls) == 1
    add_task_id, add_role, add_messages = agent_memory.add_calls[0]
    assert (add_task_id, add_role) == (request.task_id, "solver")
    assert "Use catalyst A." in add_messages[-1].content
    ...
    assert set(saved_memory_by_id) == {"agent-memory-1", "task-memory-1"}
    assert saved_memory_by_id["agent-memory-1"].score == 0.8
    assert saved_memory_by_id["agent-memory-1"].metadata["origin"] == "agent-note"
    assert saved_memory_by_id["agent-memory-1"].metadata["memory_scope"] == "agent"
    assert saved_memory_by_id["agent-memory-1"].metadata["source_backend_id"] == "agent-memory"
    assert saved_memory_by_id["task-memory-1"].score == 0.8
    assert saved_memory_by_id["task-memory-1"].metadata["origin"] == "task-note"
    assert saved_memory_by_id["task-memory-1"].metadata["memory_scope"] == "task"
    assert saved_memory_by_id["task-memory-1"].metadata["source_backend_id"] == "task-memory"
    assert saved.memory_bundle.metadata["agent_memory_state_ref"] == "agent-state-before"
    assert saved.memory_bundle.metadata["task_memory_state_ref"] == "task-state-before"
    assert saved.metadata["agent_memory_bundle"]["state_ref"] == "agent-state-before"
    assert saved.metadata["task_memory_bundle"]["state_ref"] == "task-state-before"
    assert saved.metadata["agent_memory_update_result"] == {
        "status": "updated",
        "state_ref": "agent-state-after",
    }
    assert saved.metadata["task_memory_update_result"] == {
        "status": "updated",
...
def test_default_task_runtime_records_memory_state_lineage(tmp_path: Path):
    ...
    agent_record = records[("agent-memory", "agent-state-after")]
    assert agent_record.backend_type == "memory"
    assert agent_record.created_from_task_id == request.task_id
    assert agent_record.created_from_run_ref == result["run_ref"]
    assert agent_record.parent_state_refs == ["agent-state-before"]
    assert agent_record.active is True
    assert agent_record.metadata["memory_scope"] == "agent"
    assert agent_record.metadata["memory_scope_id"] == "agent:solver"
    assert agent_record.metadata["role"] == "solver"
    assert agent_record.metadata["update_result"] == {
        "status": "updated",
        "state_ref": "agent-state-after",
    }
...
def test_default_task_runtime_records_mem0_agent_and_task_memory_lineage(tmp_path: Path):
    ...
    saved_runs = trajectory_registry.list_subagent_runs()
    assert saved_runs[0].metadata["agent_memory_bundle"]["state_ref"] == (
        "method://mem0/10:mem0-agent/5:agent/12:agent:solver/v0"
    )
    ...
    assert saved_runs[1].metadata["agent_memory_bundle"]["state_ref"] == (
        "method://mem0/10:mem0-agent/5:agent/14:agent:reviewer/v0"
    )
    ...
    assert [(record.backend_id, record.metadata["memory_scope"], record.metadata["memory_scope_id"]) for record in memory_records] == [
        ("mem0-agent", "agent", "agent:solver"),
        ("mem0-task", "task", "task:mem0-task"),
        ("mem0-agent", "agent", "agent:reviewer"),
        ("mem0-task", "task", "task:mem0-task"),
    ]
    task_records = [record for record in memory_records if record.backend_id == "mem0-task"]
    assert task_records[0].parent_state_refs == ["method://mem0/9:mem0-task/4:task/14:task:mem0-task/v0"]
    assert task_records[1].parent_state_refs == ["method://mem0/9:mem0-task/4:task/14:task:mem0-task/v2"]
...
def test_default_task_runtime_runs_each_configured_role_in_order(tmp_path: Path):
    ...
    assert [call[1] for call in memory.add_calls] == ["solver", "task", "reviewer", "task"]
...
def test_default_task_runtime_records_valid_memory_artifacts_without_failing_on_invalid_refs(
    ...
    agent_record = backend_state_registry.get_state("agent-state-after")
    assert agent_record is not None
    assert agent_record.artifact_refs == [valid_artifact]
    assert agent_record.metadata["invalid_artifact_refs"] == [{"uri": "file:///missing-type"}]
...
def test_default_task_runtime_rejects_partial_memory_scope_bindings(
