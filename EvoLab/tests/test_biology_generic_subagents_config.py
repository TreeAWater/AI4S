...
NEW_CONFIG_PATH = ROOT / "configs" / "biology_component_extraction_v1_generic_subagents.yaml"
TWO_PAPER_CONFIG_PATH = ROOT / "configs" / "biology_component_extraction_v1_two_paper_work_items.yaml"
...
def test_only_short_generic_biology_config_exists() -> None:
    ...
    assert not (ROOT / "configs" / "biology_component_extraction_v1.yaml").exists()
...
def test_generic_config_is_human_short_form_not_internal_runtime_schema() -> None:
    ...
    assert list(config["subagents"]) == ALLOWED_GENERIC_AGENT_TYPES
    assert set(config) >= {"task", "meta_agent", "subagents", "work_item_routing", "lab_root", "backends"}
...
def test_short_config_subagents_are_name_and_system_prompt_only() -> None:
    config = _load_yaml_config(NEW_CONFIG_PATH)
    subagents = config["subagents"]

    assert list(subagents) == ALLOWED_GENERIC_AGENT_TYPES
    assert "OtherAgent" not in subagents
    assert "solver" not in subagents
    for legacy_role in LEGACY_BIOLOGY_ROLE_NAMES:
        assert legacy_role not in subagents

    for name, payload in subagents.items():
        assert set(payload) == {"system_prompt"}, name
...
def test_short_config_backends_are_declared_without_exposing_runtime_task_config() -> None:
    ...
    assert "mem0-agent-memory" in config["backends"]["memory"]
...
def test_compiler_provides_default_scientific_tools_without_user_config_complexity() -> None:
    ...
    for role in roles.values():
        assert set(role["allowed_tools"]) == expected_tools
...
def test_two_paper_config_compiles_required_work_items_and_runtime_budget() -> None:
    ...
    assert source_config["subagents"]["DesignAgent"]["allowed_tools"] == ["write_report"]
    assert list(llm_backends) == ["openrouter-deepseek-v4-flash"]
    assert llm_backends["openrouter-deepseek-v4-flash"] == {
        "type": "api",
    ...
    assert task_config.roles["DesignAgent"].allowed_tools == ["write_report"]
...
def test_full_run_config_article_package_paths_exist() -> None:
    full_config_path = ROOT / "configs" / "biology_component_extraction_v1_28_article_work_items.yaml"
