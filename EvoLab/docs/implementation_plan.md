...
- `MethodMemoryBackend` + native mem0 method 可用；CI clean-run 通过 fake extraction LLM 和 fake embeddings 覆盖 native mem0，`FakeMemoryBackend` 仅保留给 focused unit tests。
...
- **ziyi owns Memory:** Memory contracts, fake/null memory backends, native mem0 method, scope mapping, memory update records, memory state refs, memory tests and docs.
...
Implement config loader, meta-agent dispatch parse/validate/retry, fake subagent loop, no-fixed-DAG integration test.
...
Implement native mem0 method, scope-aware retrieval/write mapping, targeted method tests, memory update summaries.

Implement GraphSkillBackend seed graph, node/edge schema, graph version refs, retrieval by tags/text, update summary writes.

Add backend factory/config wiring for MVP backends while keeping fake/null backends available for focused tests.

Native mem0 method and graph backend tests pass through the same contracts used by fakes.
...
- [ ] Freeze task/runtime contracts: `TaskConfig`, `TaskState`, `DispatchDecision`, `RoleSpec`, `RuntimePolicy`, `PromptBuildRequest`.
...
- [ ] Implement config loader for static roles, backend bindings, prompt builders, tool policy, and max dispatch/tool step limits.
- [ ] Implement MetaAgent dispatch parsing, schema validation, invalid-role rejection, retry recording, and terminal actions.
...
- [x] Implement native mem0 `MemoryMethod` behind `MethodMemoryBackend` for scoped search/add calls.
- [x] Map EvoLab memory scopes to native mem0 filters/state refs and record memory update summaries in Lab.
...
- [x] Keep fake/null backends available for deterministic unit tests.
- [x] Checkpoint: native mem0 method tests and graph backend retrieval/update tests pass.
### **Day 8 — API LLM Backend And SFT Trainer**
...
- [ ] Implement `SFTTrainer` with trajectory export, a dry-run smoke-test mode, and an optional minimal `transformers` backend for local supervised fine-tuning.
- [ ] Add generation metadata capture for model ID, parameters, token usage if provided, cost if provided, and latency.
- [ ] Checkpoint: tests cover API skipped evolution and SFT exported/promoted/not-recommended/failed scenarios.
### **Day 9 — Runtime Integration With MVP Backends**

- [x] Replace fake memory path in `configs/demo_v1.yaml`; `configs/demo_v1_ci.yaml` exercises native mem0 with fake extraction LLM and fake embeddings.
...
- [ ] Mark V2 items explicitly: production LoRA/PEFT training, async EvolveWorker, federated consolidation, public skill governance, full resource mining, external HITL integrations.
...
- AC 10: V1 Day 7, MethodMemoryBackend with native mem0 method and GraphSkillBackend.
...
- **Backend integration risk:** CI clean-run exercises native mem0 through fake extraction and embedding runtimes; focused fake/null backends remain for deterministic unit tests.
...
- **Memory scope creep:** V1 native mem0 method only needs scoped search/add and update summaries. Public/private memory governance stays V2.

- **Training cost risk:** V1 SFT defaults to dry-run for CI and uses optional local `transformers` dependencies only when explicitly requested. Production LoRA/PEFT stays V2.
