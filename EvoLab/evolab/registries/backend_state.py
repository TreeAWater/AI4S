...
class FileBackendStateRegistry(BackendStateRegistry):
    ...
    def _has_registered_state(self, backend_id: str, state_ref: str) -> bool:
        return any(
            record.backend_id == backend_id and record.state_ref == state_ref for record in self._load_records()
        )

    def promote(self, backend_id: str, new_state_ref: str, evolution_run_ref: str) -> None:
        if not self._has_registered_state(backend_id, new_state_ref):
            raise ValueError(
