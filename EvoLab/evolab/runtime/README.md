...
- `task_runtime.py`：MetaAgent routing、subagent execution、memory/skill retrieval、tool loop、memory update。
...
6. 按 `memory_backend_bindings` 实例化 memory runtimes，包括 role、task 和可选的 MetaAgent memory。
...
每个 subagent run 的主路径：

1. 根据 role 的 `agent_memory_backend` 和 task 的 `task_memory_backend` 构造 retrieval requests。
2. 调用 memory runtime `search()`，把返回的 `MemoryBundle` 放入 prompt context 和 trajectory。
3. 执行 LLM/tool loop。
4. 将本轮 messages 写回 agent/task memory backend 的 `add()`。
...
- CLI clean-run 应可在 fresh lab root 上重建 config、inputs、registries 和 artifacts。
