...
class DynamicWorkflowPlanner:
    ...
    def plan(
        ...
        *,
        ...
        static_subagents: list[dict[str, Any]] | None = None,
    ) -> DynamicPlanningOutcome:
        messages = self._messages(request=request, work_item=work_item, static_subagents=static_subagents or [])
        ...
        for attempt_index in range(self.config.max_planner_retries + 1):
                        ...
                        content=json.dumps(
                            {
                                ...
                                "required_response": "Return a corrected DynamicWorkflowSpec JSON object only.",
                            },
                            sort_keys=True,
                        ),
                    ),
            ...
            try:
                ...
                last_report = DynamicWorkflowValidationReport(
                    ...
                    planner_backend_id=self.config.planner_backend.backend_id if self.config.planner_backend else None,
                    default_worker_backend_id=self.config.default_worker_backend.backend_id if self.config.default_worker_backend else None,
                    metadata={"attempt_index": attempt_index, "raw_output_prefix": raw_output[:240]},
                )
                continue
    ...
    def _messages(
        ...
        *,
        ...
        static_subagents: list[dict[str, Any]],
    ) -> list[Message]:
        payload = {
            "task_id": request.task_id,
            ...
            "static_fallback_subagents": static_subagents,
            "required_response": {
                "format": "JSON only",
...
class DynamicSubAgentFactory:
    ...
    def create(
        ...
        *,
        ...
        worker_backend_id = spec.llm_backend_id or default_worker
        ...
        tool_names = _dedupe(spec.allowed_tools)
        ...
        role = RoleSpec(
            ...
            llm_backend=BackendBinding(backend_id=worker_backend_id),
            agent_memory_backend=self.agent_memory_backend,
        ...
        provenance = {
            ...
            "static_fallback_available": static_fallback_available,
...
def validate_dynamic_workflow_spec(
    ...
    *,
    ...
    report = DynamicWorkflowValidationReport(
        ...
        metadata={"workflow_id": spec.workflow_id, "work_item_id": spec.work_item_id},
    )
...
def persist_dynamic_workflow_artifacts(
    *,
    ...
    root = lab_root / "dynamic_workflows" / _safe_ref(task_id) / _safe_ref(workflow_id)
...
def _validate_tool_names(
    ...
    *,
    ...
    for tool_name in tool_names:
        ...
        elif tool_runtime._registry.get_spec(tool_name) is None:  # validated access; dynamic agents still execute via ToolRuntime.
