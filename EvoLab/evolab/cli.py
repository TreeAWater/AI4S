...
def run_clean_demo(config_path: Path | str, lab_root: Path | str | None = None) -> dict[str, Any]:
    ...
    shutil.copy2(config_path, layout.root / "configs" / config_path.name)
    _copy_skill_graph_files(config, config_path.resolve().parent, layout.root)

    task_config = _model_from_payload(TaskConfig, config["task_config"])
    task_config = _copy_meta_agent_instruction(
        task_config=task_config,
        config_dir=config_path.resolve().parent,
        lab_root=layout.root,
    )
    ...
    worker = TaskWorker(
        ...
        skill_backends=_build_skill_backends(config, config_dir=layout.root / "configs"),
...
def _compile_experiment_config(config: dict[str, Any], *, config_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(subagents_payload, dict) or not subagents_payload:
        raise ValueError("short experiment config requires non-empty subagents mapping")
    ...
    agent_memory_backend_id = _default_backend_id(backends, "memory", preferred=("mem0-agent-memory", "agent"))
    ...
    for name, payload in subagents_payload.items():
        if not isinstance(name, str) or not name:
            raise ValueError("subagent names must be non-empty strings")
        if not isinstance(payload, dict):
            raise ValueError(f"subagent {name!r} must be a mapping")
        system_prompt = payload.get("system_prompt")
        if not isinstance(system_prompt, str) or not system_prompt.strip():
            raise ValueError(f"subagent {name!r} requires system_prompt")
        role_map[name] = RoleSpec(
            ...
            llm_backend=_backend_binding_from_config(
                ...
                field_name=f"subagents.{name}.llm_backend",
            ),
            agent_memory_backend=_backend_binding_from_config(
                payload.get("agent_memory_backend"),
                default_backend_id=agent_memory_backend_id,
                field_name=f"subagents.{name}.agent_memory_backend",
            ),
    ...
    task_config = TaskConfig(
        ...
        meta_agent=MetaAgentSpec(
            ...
            system_prompt=str(meta_agent_payload["system_prompt"]),
            llm_backend=_backend_binding_from_config(
        ...
        roles=role_map,
        dynamic_subagents=_dynamic_subagents_from_config(config, backends),
...
def _dynamic_subagents_from_config(config: dict[str, Any], backends: dict[str, Any]) -> DynamicSubagentsConfig | None:
    ...
    if not parsed.enabled or parsed.mode == "static":
        return parsed
    ...
    missing = [backend_id for backend_id in (planner_id, worker_id) if backend_id and backend_id not in llm_backends]
    if missing:
...
def main(argv: list[str] | None = None) -> int:
    ...
    clean_run = subparsers.add_parser("clean-run")
    clean_run.add_argument("config", nargs="?", default="configs/demo_v0.yaml")
...
def _copy_meta_agent_instruction(
    *,
    ...
    if task_config.meta_agent is None or task_config.meta_agent.instruction_ref is None:
        return task_config
    instruction_ref = task_config.meta_agent.instruction_ref
    source = _resolve_config_ref(config_dir, instruction_ref)
    destination = _lab_relative_path(lab_root, _lab_config_ref(instruction_ref, source))
    destination.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(source, destination)
    return task_config.model_copy(
        update={
            "meta_agent": task_config.meta_agent.model_copy(
                update={"instruction_ref": str(destination)}
            )
        }
    )
