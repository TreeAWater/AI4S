        ...
        *,
        ...
        try:
            ...
            if isinstance(output, ToolResult):
                return output.model_copy(update={"call_id": call_id})
            if not isinstance(output, str):
                raise TypeError(
                    f"tool {name!r} returned non-string output: "
                    f"{type(output).__name__}"
                )
            return ToolResult(call_id=call_id, status="ok", content=output)
        except Exception as exc:
            return ToolResult(call_id=call_id, status="error", content=str(exc))

    def _get_effective_spec(self, name: str) -> ToolSpec | None:
        return self._runtime_specs.get(name) or self._registry.get_spec(name)
