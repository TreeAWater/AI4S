...
def register_file_tools(registry: ToolRegistry, *, base_dir: str | Path | None = None) -> None:
    _register_if_missing(
        registry,
        _list_files_spec(),
        lambda arguments: list_files(resolve_path_arguments(arguments, base_dir=base_dir, names=("root",))),
    )
    _register_if_missing(
        registry,
        _read_text_spec(),
        lambda arguments: read_text(resolve_path_arguments(arguments, base_dir=base_dir, names=("path",))),
    )
    _register_if_missing(
        registry,
...
def list_files(arguments: dict[str, Any]) -> ToolResult:
    ...
    for path in sorted(
        _iter_file_candidates(search_roots, recursive=recursive, warnings=warnings),
...
def read_text(arguments: dict[str, Any]) -> ToolResult:
...
def inspect_file_metadata(arguments: dict[str, Any]) -> ToolResult:
...
def _iter_file_candidates(search_roots: list[Path], *, recursive: bool, warnings: list[str]) -> list[Path]:
    ...
    for search_root in search_roots:
        ...
        if not search_root.exists():
            continue
        if recursive:
            candidates.extend(_walk_files(search_root, warnings=warnings))
        else:
            candidates.extend(_direct_child_files(search_root, warnings=warnings))
    return candidates


def _walk_files(root: Path, *, warnings: list[str]) -> list[Path]:
    files: list[Path] = []
...
def _direct_child_files(root: Path, *, warnings: list[str]) -> list[Path]:
    try:
        return sorted((item for item in root.iterdir() if item.is_file()), key=lambda item: item.as_posix())
    except OSError as exc:
